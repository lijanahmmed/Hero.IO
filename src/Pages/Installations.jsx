...
const Installations = () => {
  ...
  const [installApplication, setInstallApplication] = useState(() =>
    loadAppInstallation()
  );
    ...
    <div className="mt-15 w-11/12 md:w-10/12 mx-auto">
      ...
      {installApplication.length > 0 ? (
        <div className="mt-10 space-y-4 pb-20">
          {sortedApp.map((installation) => (
            <Installation
              key={installation.id}
              installation={installation}
              setInstallApplication={setInstallApplication}
            ></Installation>
          ))}
        </div>
      ) : (
        <p className="mt-15 text-center text-3xl font-bold pb-20">
